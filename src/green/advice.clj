(ns green.advice
  ...
  Registries are plain maps of step-name -> vector of {:id :how :fn},
  so all operations are pure — advice is workflow-scoped, not global.")

(defn add
  "Register advice `f` on `step` with combinator `how` under `id`.
  Re-adding an existing id replaces it and moves it to the top of the stack."
  [registry step how id f]
